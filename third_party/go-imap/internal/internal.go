...
func ExpectFlag(dec *imapwire.Decoder) (imap.Flag, error) {
	...
	// Tolerate a malformed flag whose name begins with further backslashes,
	// such as the "\\Seen" keyword some clients create. RFC 3501 does not
	// permit it, but real mailboxes contain it, and refusing to parse the
	// response makes the entire mailbox unusable rather than just that flag.
	extra := 0
	for dec.Special('\\') {
		extra++
	}
